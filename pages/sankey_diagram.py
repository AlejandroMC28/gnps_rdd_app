...
simple_complex_flows = st.sidebar.selectbox("Select Food Type for flow visualization", ["all", "simple", "complex"])
groups_for_flows = st.sidebar.multiselect("Select Groups for flow visualization", [f'G{i}' for i in range(1, 7)], ['G1', 'G4'])
max_hierarchy_flows = st.sidebar.slider("Select Max Hierarchy Level", 0, 6, 4)

hierarchical_levels_file = st.file_uploader("Upload Hierarchy File", type=['txt', 'csv', 'tsv'])
gnps_network_file = st.file_uploader("Upload your molecular network file", type=["csv", "tsv"])

if hierarchical_levels_file and gnps_network_file:
