...
def get_dataset_food_counts_all(
    ...
    """
    #file_content = gnps_network.getvalue()
    #st.text(f"File content:\n{file_content.decode('utf-8')[:800]}...")  # Display first 200 characters of the file
    
    #gnps_network_df = pd.read_csv(gnps_network, sep='\t' if gnps_network.name.endswith('.tsv') else ',')
    metadata = get_sample_metadata(gnps_network, all_groups)
